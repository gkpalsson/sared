...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [FVr_bestmem,S_bestval,I_nfeval] = deopt_grid(fname,S_struct)
...
FVr_res      = S_struct.res;
par          = S_struct.parallel;
...
format long
FM_pop     = roundnv(FM_pop,FVr_res);
GKPbestmem = roundnv(S_struct.GKP_bestmem,FVr_res);
...
FM_pop(I_best_index,:) = GKPbestmem;
...
if par == 1
  parfor k=1:I_NP      
    S_tempval_gkp(k) = feval(fname,FM_pop(k,:),S_struct);   % check cost of competitor
  end
else
  for k=1:I_NP      
    S_tempval_gkp(k) = feval(fname,FM_pop(k,:),S_struct);   % check cost of competitor
...
oldbest =  S_bestval.FVr_oa(1);

% Modification by gkp to include a best initial guess.
%I_best_index   = 1;
%FM_pop(1,:)   = S_struct.GKP_bestmem;
%S_bestval     = feval(fname,S_struct.GKP_bestmem,S_struct);
%FVr_bestmemit = S_struct.GKP_bestmem;
%S_bestvalit   = S_bestval;
%FVr_bestmem   = S_struct.GKP_bestmem;
...
FM_ui = roundnv(FM_ui,FVr_res);
FM_origin = roundnv(FM_origin,FVr_res);
      ...
      if (I_bnd_constr == 1)
         for j=1:I_D %----boundary constraints via bounce back-------
            if (FM_ui(k,j) > FVr_maxbound(j))
               FM_ui(k,j) = roundn(FVr_maxbound(j) + rand*(FM_origin(k,j) - FVr_maxbound(j)),FVr_res(j));
            end
            if (FM_ui(k,j) < FVr_minbound(j))
               FM_ui(k,j) = roundn(FVr_minbound(j) + rand*(FM_origin(k,j) - FVr_minbound(j)),FVr_res(j));
            end   
         end
      end
...
if par == 1
  parfor k=1:I_NP      
    S_tempval(k) = feval(fname,FM_ui(k,:),S_struct);   % check cost of competitor
  end
else
  for k=1:I_NP      
    S_tempval(k) = feval(fname,FM_ui(k,:),S_struct);   % check cost of competitor
...
for k = 1:I_NP
      I_nfeval  = I_nfeval + 1;
      if (left_win(S_tempval(k),S_val(k)) == 1)   
         FM_pop(k,:) = FM_ui(k,:);                    % replace old vector with new one (for new iteration)
         S_val(k)    = S_tempval(k);                      % save value in "cost array"
      
         %----we update S_bestval only in case of success to save time-----------
         if (left_win(S_tempval(k),S_bestval) == 1)   
            S_bestval = S_tempval(k);                    % new best value
...
%----Output section----------------------------------------------------------
  ...
  if (I_refresh > 0)
     if (S_bestval.FVr_oa(1) ~= oldbest || I_iter == 1)
