...
try [xl,xh]=expandarrays(xerr{1},xerr{2}); 
catch
  error('xl and xh are not consistent in size.'), 
end
try [y,xl,ty,txl]=expandarrays(y,xl); 
catch
...
try [y,xh]=expandarrays(y,xh); 
catch
  error('xh and y are not consistent in size.'), 
end
if relxerr
    try [xl,x,txl]=expandarrays(xl,x); 
    catch
      error('xl and x are not consistent in size.'),
    end
    if txl, xh=xh'; end
    try [x,xh]=expandarrays(x,xh); 
    catch
...
try [yl,yh]=expandarrays(yerr{1},yerr{2}); 
  ...
  error('yl and yh are not consistent in size.'),
end
try [x,yl,tx,tyl]=expandarrays(x,yl); 
catch
